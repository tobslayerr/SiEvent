...
const Register = () => {
  ...
  const handleRegister = (e) => {
    ...
    if (!fullName || !username || !email || !password) {
      toast.error("Harap isi semua data!", { position: "top-right", autoClose: 3000 });
      return;
    }

    if (!validateUsername(username)) {
      toast.error("Username harus minimal 5 karakter & tidak boleh ada spasi!", { position: "top-right", autoClose: 3000 });
      return;
    }

    if (!validateEmail(email)) {
      toast.error("Format email tidak valid!", { position: "top-right", autoClose: 3000 });
      return;
    }

    if (passwordStrength !== "Kuat") {
      toast.error("Gunakan password yang lebih kuat!", { position: "top-right", autoClose: 3000 });
      return;
    }

    if (!captchaChecked) {
      toast.error("Harap verifikasi CAPTCHA!", { position: "top-right", autoClose: 3000 });
      return;
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen px-4">
      ...
      <div className="bg-gray-100 p-6 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form onSubmit={handleRegister}>
          ...
          <button
            type="submit"
            className="w-full bg-black text-white py-2 rounded-full font-medium transition hover:bg-gray-200 hover:text-black border border-black active:scale-90 duration-300 mt-4"
          >
            Register
          </button>
        </form>
        ...
        <p className="text-center mt-3 text-sm text-gray-600">
          Sudah punya akun? <a href="/login" className="text-black font-medium hover:underline">Login</a>
        </p>
        <button
          onClick={() => navigate("/")}
