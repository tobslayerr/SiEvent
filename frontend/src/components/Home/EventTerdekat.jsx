    ...
    <div className="max-w-6xl mx-auto px-4 pt-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {events.map((event, index) => (
          <div
            key={event.id}
            className={`bg-white rounded-lg shadow-md overflow-hidden cursor-pointer transition duration-300 transform hover:shadow-lg hover:scale-105 active:scale-98 ${
              index === 4 ? "hidden sm:block md:block lg:block xl:block" : ""
            }`}
          >
            <img src={event.image} alt={event.title} className="w-full h-40 object-cover transition active:scale-95" />
            <div className="p-3">
